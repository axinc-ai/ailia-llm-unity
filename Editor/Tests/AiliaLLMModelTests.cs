...
public class AiliaLLMModelTests
{
    ...
    {
        // Test that Generate() returns false when SetPrompt() has not been called.
        // The native library should return AILIA_LLM_STATUS_INVALID_STATE in this case.
        model.Create();

        bool done = false;
        bool result = model.Generate(ref done);

        Assert.IsFalse(result, "Generate without SetPrompt should return false");
    }
